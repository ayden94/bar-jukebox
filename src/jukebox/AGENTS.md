...
| Schema or persistence changes | db.ts | saveSnapshot rewrites queue/history/now_playing wholesale in one transaction |
...
- db.ts opens jukebox.sqlite relative to the process cwd — run the server from the repo root.

## ANTI-PATTERNS
- Never write SQLite outside db.ts; never touch Music.app outside playback.ts.
