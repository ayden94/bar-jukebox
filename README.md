...
- **재생 루프**: `osascript`로 Music.app 제어 — 앨범 `music://` 오픈 후 뮤트 상태에서 트랙 스킵, `play once`로 요청 곡 1곡만 재생 대기에 남기고 볼륨 복구. 재생 위치는 1초 간격으로 SSE에 실시간 반영 (Music.app의 자동 재생을 꺼두면 종료 후 추천 곡이 큐에 쌓이지 않아요)
- **실시간**: `@Sse`(`/api/events`) — 어드민 순서 변경/재생 상태가 손님 화면에 즉시 반영 (폴링 폴백 내장)

## 룰

- **신청 곡 수 제한** (운영 설정, 기본값 기기당 1곡·테이블당 5곡, 0은 무제한): 신원은 서버 발급 쿠키(`bj_did`, httpOnly, 1년) 기준. 기기/테이블별로 큐+재생중 곡 수를 합산해 상한을 넘으면 409로 차단
